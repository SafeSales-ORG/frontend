...
type Step = 'welcome' | 'generate' | 'secure' | 'profile' | 'login' | 'connect' | 'choose';
...
const AuthDialog: React.FC<AuthDialogProps> = ({ isOpen, onClose }) => {
  ...
  const [profileData, setProfileData] = useState({ name: '', about: '', picture: '', email: '', accountNumber: '' });
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      setProfileData({ name: '', about: '', picture: '', email: '', accountNumber: '' });
  ...
  const finishSignup = async (skipProfile = false) => {
    try {
      // Demo mode has no Nostr relay to publish a kind-0 profile to; the
      // SafeSale seller record already carries name/avatar, so skip it (and
      // the misleading "profile could not be saved" error it would throw).
      if (
        !DEMO_MODE &&
        !skipProfile &&
        ...
        if (profileData.email) metadata.email = profileData.email;
        if (profileData.accountNumber) metadata.account_number = profileData.accountNumber;
  ...
  const goSeller = () => {
    onClose();
    // Selling requires a SafeSale seller record, which this dialog doesn't
    // create — send them through onboarding (which calls createSeller +
    // setCurrentSeller) instead of straight to /app, where SellerGate would
    // otherwise bounce them to "open your shop".
    navigate('/onboarding');
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[95vw] sm:max-w-sm max-h-[90dvh] p-0 gap-0 overflow-hidden rounded-2xl overflow-y-auto">
        ...
        <div className="px-6 pb-6 pt-4 space-y-5">
          ...
          {/* Choose step — buyer vs seller, shown right after auth succeeds. */}
          {step === 'choose' && (
            <div className="space-y-4">
              ...
              <button
                type="button"
                onClick={goBuyer}
                className="flex w-full items-center gap-3 rounded-xl border border-border bg-white p-4 text-left transition-colors hover:border-brand/50 hover:bg-surface"
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-xs text-ink-soft">
                    Browse the marketplace and buy safely with escrow.
                  </span>
                </span>
              </button>
              <button
                type="button"
                onClick={goSeller}
                className="flex w-full items-center gap-3 rounded-xl border border-brand/30 bg-brand-soft/40 p-4 text-left transition-colors hover:border-brand/60"
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-xs text-ink-soft">
                    Open your shop and start listing in seconds.
                  </span>
                </span>
              </button>
          ...
          {step === 'profile' && (
            <div className="space-y-4">
              ...
              <div className={`space-y-4 ${isPublishing ? 'opacity-50 pointer-events-none' : ''}`}>
                ...
                <div className="space-y-1.5">
                  <label htmlFor="profile-account" className="text-sm font-medium">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setProfileData((prev) => ({ ...prev, accountNumber: e.target.value }))
                    }
